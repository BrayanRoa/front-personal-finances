...
export class DashboardControlComponent implements OnInit {
  ...
  getTransactions(page: number = 1, search: string = "", year: number = new Date().getFullYear(), month: number = new Date().getMonth() + 1, order:string = '', asc:string = 'false') {
    page = page || 1; // Si no se pasa página, se toma la 1era por defecto

    this.transactionsService.getTransactions({ walletId: 1, page, per_page: 10, year, month, search, order, asc }).subscribe({
      next: (transactions) => {
